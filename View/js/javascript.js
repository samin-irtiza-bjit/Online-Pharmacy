...
function validateLogin() {
    ...
    if (((/^[0-9a-zA-Z\_]+$/).test(user_login))) {
        if (((/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,20}$/).test(pass))) {
            value = 2;
        } else {
                ...
                if ((/^[0-9a-zA-Z\-\,\.\s\-\#]+$/).test(address)) {
                    if (((/^[0-9a-zA-Z\.\s]+$/).test(first_name)) && ((/^[0-9a-zA-Z\s\.]+$/).test(last_name))) {
                        if (((/^[0-9a-zA-Z\_]+$/).test(username))) {
                            if (((/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,20}$/).test(pass)) && ((/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,20}$/).test(retype))) {
                                alert("Registration Successful!");
                                value = 2;
                            } else {
                                document.getElementById('pass').style.borderBottom = "1px solid #DEDEDE";
                                alert("Failed: Password must be at least 8 characters and must contain at least one lower case character, one upper case character, one digit and one special character.");
                                value = 1;
                            }
                        } else {
...
$(document).ready(function () {
    $("#submit-change-pass").click(function () {
        if (($("#current-pass").val() == "") || ($("#new-pass").val() == "") || ($("#confirm-new-pass").val() == "")) {
            ...
            if ((/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,20}$/).test($("#new-pass").val()) && $("#new-pass").val().length >= 8) {
