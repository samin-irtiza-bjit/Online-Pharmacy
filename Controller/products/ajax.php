<?php
    ...
    if (isset($_POST['display_products'])) {
        if ($_POST['cat_id'] == "all") {
?>
    <h3 style="color:#333333;">Product List</h3>
<?php
        } else {
            if ($user->get_session()) {
?>
...
?>
    <table class="table table-striped table-hover font-roboto" <?php if ($user->get_session()) { ?> id="products-session" <?php } else { ?> id="products-nosession" <?php } ?>>
        <thead>
            <tr>
                <th>Product</th>
...
<?php 
        if ($user->get_session()) {
?>
                <th style="width: 10%;" class="ta-right price_unit-title">Unit Price</th>
<?php
        }
?>
            </tr>
...
<?php 
                    if ($user->get_session()) {
?>
                    <td class="bold price_unit" style="text-align: right;"><?php echo number_format((float) $values['pro_unit_price'], 2, '.', ','); ?></td>
<?php 
                    }
?>
            </tr>
...
<?php 
                if($user->get_session()){
?>
                <td class="bold price_unit" style="text-align: right;">
                <?php echo number_format ((float)$values['pro_unit_price'], 2, '.', ',');?></td>
<?php 
                }
?>
            </tr>
