...
<div class="card-wrapper w-80" style="margin-top:80px;">
    <div class="card-style w-100">
        ...
        <div class="notif-content">
            <h3 style="width: 100%; margin: 0px 0px 8px 0px;float: left; color:#333;"><?php echo $data['n_title']; ?></h3>
            <span style="font-size: 13px; color: rgba(0,0,0,0.6); font-weight: 400;">
            <?php 
                $date = new DateTime($data['n_date_added']);
                echo $date->format('F j, Y'); ?> at <?php $date = new DateTime($data['n_time_added']);
                echo $date->format('H:iA'); ?></span>
            <hr style="padding:0.5px;">
            <div class="notif-body">
                <label>Subject</label></br>
                <p style="margin: 0px 10px 16px 0px;">
            <?php 
                if ($data['n_subject']) {
                    echo $data['n_subject'];
                } else {
