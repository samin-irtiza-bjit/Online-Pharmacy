<?php
    ...
    class Users {
        ...
        public function get_client($id) {
            $sql = "SELECT client_id FROM tbl_client WHERE client_id = '$id'";
            $result = mysqli_query($this->db, $sql);
            $row = mysqli_fetch_assoc($result);
            $value = $row['client_id'];
            return $value;
        ...
        public function new_user_request($ContactNumber, $EmailAddress, $Username, $Password, $FirstName, $LastName) {
            $sql = "SELECT * FROM tbl_client WHERE usr_username = '$Username'";
            $check = $this->db->query($sql);
            $count_row = $check->num_rows;
            if ($count_row == 0) {
                $newpassword = md5($Password);
                $sql = "INSERT INTO tbl_client(contact_no, email, usr_username, usr_password, usr_firstname, usr_lastname)
                VALUES('$ContactNumber', '$EmailAddress', '$Username', '$newpassword', '$FirstName', '$LastName')";
        ...
        public function edit_prof($contact, $email, $usr_id) {
            $sql = "UPDATE tbl_client SET contact_no='$contact', email='$email' WHERE client_id='$usr_id'";
            $result = mysqli_query($this->db, $sql) or die(mysqli_error() . "Cannot Insert Data");
            return $result;
        }
        ...
        public function receive_order_notif($id) {
            $sql = "INSERT INTO tbl_notifications(n_date_added,n_time_added,n_title,n_subject,n_message,n_status,client_id) VALUES(NOW(),NOW(),'Order Received','Pending','Your order has been placed. We will notify you once your order has been approved. Thank you for shopping at PharmEasy.','0','$id')";
            $result = mysqli_query($this->db, $sql) or die(mysqli_error() . "Cannot Insert Data");
            return $result;
        }
